...
def generate_book(ws, data, formatt, merged_format):
    ...
    for i, g in enumerate(data):
        ...
        ws.merge_range(row, 0, row +  1, 0, "Clave \ Entidad ", formatt)
        clss = 0
        for cl in range(0, cols, 4):
            crow = row
            print(g.get("entities"))
            print("COLS: {0} - {1}".format(clss, cols))
            ws.merge_range(row, cl + 1, row, cl + 4, g.get("entities")[clss], merged_format)
...
keyss = [{"it":["'machine learning'"], "keys":["machine","learning"]},
             {"it": ["'machine learning'","python"], "keys": ["machine","learning","python"]},
             {"it": ["'machine learning'","python", "algorithms"], "keys": ["machine","learning","python", "algorithms"]}]
